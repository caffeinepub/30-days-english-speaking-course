...
interface NonInfExample {
  english: string;
  hindi: string;
  highlight?: string;
}

interface NonInfRule {
  id: string;
  title: string;
  subtitle: string;
  color: string;
  ...
  hindiRule: string;
  structure: string;
  examples: NonInfExample[];
}

const nonInfRules: NonInfRule[] = [
  {
    id: "modal",
    title: "After Modal Auxiliaries",
    subtitle:
      "can, will, shall, may, must, should, would, could, might, ought to",
    ...
    hindiRule:
      "Modal auxiliary verbs (can, will, shall, may, must आदि) के बाद हमेशा bare infinitive — 'to' के बिना — उपयोग होती है।",
  ...
  },
  {
    ...
    hindiRule:
      "Causative verbs 'let', 'make', और 'have' के बाद bare infinitive उपयोग होती है। ये verbs किसी को कुछ करवाने या करने देने को दर्शाते हैं।",
  ...
  },
  {
    ...
    hindiRule:
      "'help' के बाद bare infinitive या to-infinitive दोनों सही हैं। spoken English में bare infinitive अधिक प्रयोग होती है।",
  ...
  },
  {
    ...
    hindiRule:
      "Emphatic statements, negative sentences, और 'do' या 'did' वाले प्रश्नों में हमेशा bare infinitive उपयोग होती है।",
  ...
  },
  {
    ...
    hindiRule:
      "Perception verbs (see, hear, watch, feel, notice) के बाद पूरी क्रिया के लिए bare infinitive उपयोग होती है।",
...
function NonInfCard({ rule, index, expandedId, onToggle }: CardProps) {
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {!isExpanded && (
              <p className="font-body text-xs text-muted-foreground mt-1 max-w-[260px] truncate">
                {rule.subtitle}
              </p>
            )}
          </div>
        ...
        <div
          ...
          style={{
            background: isExpanded
              ? `${rule.color.replace("oklch(", "").replace(")", "")} / 15%)`
              : "oklch(0.93 0.01 265)",
...
export default function NonInfinitivePage() {
  const [expandedId, setExpandedId] = useState<string | null>("modal");
  const [activeTab, setActiveTab] = useState<
    "all" | "modal" | "let-make" | "help" | "do-did" | "perception"
  >("all");

  const filtered =
    activeTab === "all"
      ? nonInfRules
      : nonInfRules.filter((r) => r.id === activeTab);
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      >
        ...
        <div>
          <h1 className="font-display text-3xl md:text-4xl font-extrabold text-foreground">
            Non-Infinitive (Bare Infinitive)
          </h1>
          <p className="text-muted-foreground font-body mt-0.5">
            Base verb WITHOUT 'to' — 5 usage rules with Hindi translations
          </p>
        </div>
      </motion.div>
      ...
      >
        ...
        <p className="font-body text-base text-foreground leading-relaxed mb-2">
          A <strong>bare infinitive</strong> (also called non-infinitive) is the{" "}
          <strong>base form of a verb used WITHOUT 'to'</strong>. It appears
          after modal auxiliaries, causative verbs, and perception verbs.
        </p>
        <p
          className="font-body text-sm italic"
          style={{ color: "oklch(0.52 0.06 265)" }}
        >
          <strong>Bare Infinitive</strong> (गैर-infinitive) क्रिया का वह मूल रूप है
          जो <strong>'to' के बिना</strong> उपयोग होता है। यह modal auxiliaries,
          causative verbs, और perception verbs के बाद आता है।
        </p>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mt-4">
          ...
          >
            ...
            <div className="font-mono text-sm font-semibold text-foreground">
              She wants{" "}
              <span style={{ color: "oklch(0.50 0.24 160)" }}>to sing</span>.
            </div>
            <div
              className="font-body text-xs italic mt-1"
              style={{ color: "oklch(0.52 0.06 265)" }}
            >
              Woh gaana chahti hai.
            </div>
          </div>
          <div className="rounded-xl p-4" style={{ background: BG_COLOR }}>
            <div
              className="text-xs font-bold uppercase tracking-widest mb-2"
              style={{ color: COLOR }}
            >
            ...
            <div className="font-mono text-sm font-semibold text-foreground">
              She can <span style={{ color: COLOR }}>sing</span>.
            </div>
            <div
              className="font-body text-xs italic mt-1"
              style={{ color: "oklch(0.52 0.06 265)" }}
      ...
      >
        <div
          ...
          style={{ color: COLOR }}
        >
          Quick Reference — When to Use Bare Infinitive / Sankhipt Sandarbh
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            ...
            <div key={item.verb} className="bg-white rounded-xl p-4 shadow-sm">
              <div
